  <head>
     <meta charset="utf-8">
     <link rel="stylesheet" type="text/css" href="http://localhost/web_project/style.css">
</head>
  
  <header>
    <img src="data:image/jpeg;base64," alt="naver_shopping_image">
        
       
  </header>
<?php


$connect = mysqli_connect("localhost", "root", "458658", "scrapy");
// $query = "select sentValue, count(*) as number from se group by sentValue WHERE model = NULL";
// $query = "select *  from totalscore where model like '%"+$_GET[model8]+"%'";
// $query = "select *  from totalscore where model like '%네오티즌%'";
    if($_GET[model1] == "대우어플라이언스")
	    $query = "select *  from totalscore where model like '%".$_GET[model1]."%'";
	else if($_GET[model2] == "미로")
		$query = "select *  from totalscore where model like '%".$_GET[model2]."%'";
	else if($_GET[model6] == "아로미")
		$query = "select *  from totalscore where model like '%".$_GET[model6]."%'";
	else
		echo "NO DATA";
$result = mysqli_query($connect,$query);


$connect2 = mysqli_connect("localhost", "root", "458658", "scrapy");
	if($_GET[model1] == "대우어플라이언스")
		$query2 = "select sentWord,count(*) as number from se where model like '%".$_GET[model1]."%' group by sentWord order by number desc limit 5";
	else if($_GET[model2] == "미로")
		$query2 = "select sentWord,count(*) as number from se where model like '%".$_GET[model2]."%' group by sentWord order by number desc limit 5";
	else if($_GET[model6] == "아로미")
		$query2 = "select sentWord,count(*) as number from se where model like '%".$_GET[model6]."%' group by sentWord order by number desc limit 5";
	else
		echo "NO DATA";
$result2 = mysqli_query($connect,$query2);
				
echo $_GET[model1];
echo $_GET[model2];
echo $_GET[model6];
								
								
?>

<!DOCTYPE html>
<html>
   <head>
      <script type="text/javascript" src="https://www.gstatic.com/charts/loader.js"></script>
    <script type="text/javascript">
      google.charts.load("current", {packages:["corechart"]});
      google.charts.setOnLoadCallback(drawChart);
     google.charts.setOnLoadCallback(drawChart2);
     function drawChart() {
        var data = google.visualization.arrayToDataTable([
            ['감정분석결과','점수'],
            <?php
            
            while($row = mysqli_fetch_array($result))
            {
               echo "['"."긍정"."', ".$row["positive"]."],";
               echo "['"."부정"."', ".$row["negative"]."],";
               echo "['"."중립"."', ".$row["neutral"]."],";
            }
            ?>
            ]);
       var piechart_options = {
            colors:['blue','red','gray'],
            title:'감정분석 결과 - 긍정/부정/중립 파이차트',
                width:400,
                height:300};
        var piechart = new google.visualization.PieChart(document.getElementById('piechart_div'));
        piechart.draw(data, piechart_options);
      
      }
     function drawChart2() {
        var data2 = google.visualization.arrayToDataTable([
            ['감정분석결과','점수'],
            <?php
            
            while($row2 = mysqli_fetch_array($result2))
            {
               echo "['".$row2["sentWord"]."', ".$row2["number"]."],";
            
            }
            ?>
            ]);
        var barchart_options = {
          title: '감정분석 결과 - 상위 5개 핵심 감정어',
        width:400,
        height:300
        
        };

        var barchart = new google.visualization.BarChart(document.getElementById('barchart_div'));

        barchart.draw(data2, barchart_options);
      }
      
    </script>
  </head>
  <body>
    <!--Table and divs that hold the pie charts-->
    <table class="columns">
      <tr>
        <td><div id="piechart_div" style="border: 1px solid #ccc"></div></td>
        <td><div id="barchart_div" style="border: 1px solid #ccc"></div></td>
      </tr>
    </table>
  </body>
</html>